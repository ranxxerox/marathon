...
case class DeploymentPlan(version: String, deploymentId: String)
...
class MarathonFacade(url: String, baseGroup: PathId, waitTime: Duration = 30.seconds)(implicit val system: ActorSystem) extends JacksonSprayMarshaller {
  ...
  implicit val deploymentPlanMarshaller = marshaller[DeploymentPlan]
  ...
  def killAllTasks(appId: PathId): RestResult[ListTasks] = {
    requireInBaseGroup(appId)
    val pipeline = sendReceive ~> read[ListTasks]
    result(pipeline(Delete(s"$url/v2/apps$appId/tasks")), waitTime)
  }

  def killAllTasksAndScale(appId: PathId): RestResult[DeploymentPlan] = {
    requireInBaseGroup(appId)
    val pipeline = sendReceive ~> read[DeploymentPlan]
